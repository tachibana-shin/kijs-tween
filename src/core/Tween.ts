import { Tween as TweenByTweenjs, update as updateByTweenjs, getAll as getAllByTweenjs, Easing } from "@tweenjs/tween.js"
...
class Tween<El extends any> extends TweenByTweenjs {
  ...
  constructor(elem: El, prop: string, value: number | string, options: Options<El>) {
    this.elem = elem;
    this.options = options;
    this.prop = prop;
    
    const adjust = adjustCSS(elem, prop, typeof value === "number" ? value + "px" : value, () => this.cur())
...
function createTweensByProps(el:any, props: {
...

{
  ...
  for ( const prop in props ) {
    if (rfxtypes.exec(props[prop])) {
      ...
      const type = props[prop] === "toggle" ? (isHidden ? "show" : "hide") : props[prop]
  ...
  tweens[0]?.onProgress((tween) => {
    options.progress?.call(el, tween.now / tween.end, tween.duration() * (1 - tween.now / tween.end))
  })
  }
  ...
  if (isBox) {
    ...
    tweens[0]?.onStart(() => {
      // backup overflow
      overflowsBackup = [elem.style.overflow || "", elem.style.overflowX || "", elem.style.overflowY || ""]
    ...
    promise.finally(() => {
      // restore
      [elem.style.overflow, elem.style.overflowX, elem.style.overflowY] = overflowsBackup
...
function startTweens(tweens: ReturnType<typeof createTweensByProps>["tweens"]): void {
  tweens.forEach(tween => tween.start());
  
  update();
}

const storeWeakAnimate = new WeakMap<any, Map<string, Set<Props | (() => void | Promise<void>)>>>()
const storeWeakTweenPropsRunning = new WeakMap<any, Set<ReturnType<typeof createTweensByProps>>>();
...
function startTweenQueueInStore(elem: any, queueName: string): void {
  if (storeWeakAnimate.get(elem)!.get(queueName)!.size > 0) {
    /* if (storeWeakTweenPropsRunning.has(elem)) {
      ...
      const props = storeWeakAnimate.get(elem)!.get(queueName)!.values().next().value
      
      const tweenProps = typeof props === "function" ? { promise: props() } : createTweensByProps(elem, props)
...
Kijs.prototype.delay = function(ms: number) {
    ...
    this.each(elem => {
      ...
      storeWeakAnimate.get(elem)!.get(queueName)!.add(() => new Promise<void>(resolve => setTimeout(resolve, ms)))
    })
}